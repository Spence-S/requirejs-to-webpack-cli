...
module.exports = function assemble(conversion, callback) {
    ...
    function providedPluginIterator(plugin) {
        ...
        // transform plugin to path.resolve(plugin)
        plugin = `%LITERAL%require.resolve(${plugin})%LITERAL%`;
    ...
    if (shims.providedPlugins.length > 0) {
        ...
        info(shims.providedPlugins);
        shimString =
            'const providePlugin = new webpack.ProvidePlugin(' +
            JSON.stringify(providedPlugins, false, 2) +
            ')';

        config.plugins = ['%LITERAL%providePlugin%LITERAL%'];
    }

    if (shims.rules.length > 0) {
        config.module = {
            rules: shims.rules
        };
    }
